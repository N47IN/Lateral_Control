...
while driver.step()!=-1:
   ...
   if step == 120:
      ...
      stanley = mpc_FFS.LatControl(coordinates_cartesian)
      ...
      #obs1 = llc([12.9915319, 80.231603006])[0:2]
      obs1 = llc([12.991568742470744, 80.23158389922915])[0:2]
      #obs1 = llc([12.991565582974513, 80.23163729436368])[0:2]
      ...
      steer_angle1, k = stanley.steer_control(curr_pos,yaw,speed)
      for i in obs :
         if obs_flag is False:
            obs_flag = True
            stanley.setObs(i)     
            if np.linalg.norm(curr_coord - i) > 15:
               steer_angle, k = stanley.steer_control(curr_pos,yaw,speed)
               sine_pose = curr_pos
               stanley.resetObs()
               obs_flag = False
      ...
      if obs_flag is True:
            stanley.switch(np.linalg.norm(curr_coord - i))
            steer_angle = 1.1*stanley.mpc_ffs(curr_pos[0:2],yaw,speed) 
            steer = steer_angle
